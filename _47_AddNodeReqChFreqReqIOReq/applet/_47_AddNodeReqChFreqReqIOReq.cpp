...
//TEST ADD_NODE_REQ = OK
  unsigned char ar1[2] = {0x00, 0x0F};   // mask 9 = TEMPERATURE + BATTERY 
  unsigned char forced[4] = {0xFF, 0xFE, 0x08, 0x00};
//TEST MASK_REQ = OK  
  // content = ""         
...
void setup()
{
      ...
      if( COMM.setupXBee(panID, ROUTER, gateway, NONE, "NodeC", 0, HIGHPERFORMANCE) ) 
...
void loop()
{
       ...
       */
       /// TEST ADD_NODE_REQUEST:
         ...
         er = COMM.sendMessage(dest, ADD_NODE_REQ, (char *) forced );
       /// TEST MASK_REQUEST: 
