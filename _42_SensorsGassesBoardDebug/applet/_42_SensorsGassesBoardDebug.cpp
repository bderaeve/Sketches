...
void loop()
{
      ...
      USB.print("\nFree memory "); USB.println(freeMemory());
      
      value = SensorGasv20.readValue(SENS_TEMPERATURE);
      SensorGasv20.setSensorMode(SENS_OFF, SENS_TEMPERATURE);
      USB.print("Temperature: "); USB.print(value-2.3); USB.println(" \u00b0C");
      
      value = SensorGasv20.readValue(SENS_HUMIDITY);
      SensorGasv20.setSensorMode(SENS_OFF, SENS_HUMIDITY);
      USB.print("Humidity: "); USB.print(value); USB.println(" %");
      
      //SensUtils.measureTemperature();
      //xbeeZB.setAlarmForRouter();   //DOES CAUSE ERROR
      COMM.receiveTest();  // DOES NOT CAUSE THE ERROR
      PackUtils.setPacketMask(test);
      delay(1000);
}
